...
int main()
{
	...
	unsigned char remaining_length = input_length;  //the amount of data that still needs to be processed
	unsigned char output[SHA_DIGEST_LENGTH];  //stores the final SHA-1 hash
											  //20 bytes - 160 bit hash

	while (remaining_length > 0)
	{
		// the program processes the input in blocks of INPUT_BLOCK_LENGTH
	...
	FILE* f = NULL;
	errno_t err = fopen_s(&f, "DAY1/input_SHA1.bin", "rb");
	if (err == 0)
	{
		SHA1_Init(&ctx);
	...
	FILE* g = NULL;
	errno_t err2 = fopen_s(&g, "DAY1/input2.txt", "w");
	...
	err2 = fopen_s(&g, "DAY1/input2.txt", "r");
	SHA1_Init(&ctx);
	unsigned int temp;
	unsigned char buff;
	...
	// hashing input2.txt with SHA-256
	err2 = fopen_s(&g, "DAY1/input2.txt", "r");
	SHA256_CTX ctx2;
	SHA256_Init(&ctx2);
